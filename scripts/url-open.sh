...
var=`cat /tmp/yarn-log | tail -1 | awk -F'         ' '{ print $1 }'` #Split the strings if they have been looped, only use the first one.
...
#For a one-liner:
#chromium $(cat /tmp/yarn-log | tail -1 | awk -F'         ' '{ print $1 }' | grep http | sed 's/http/\nhttp/g' | grep ^http | sed 's/\(^http[^ <]*\)\(.*\)/\1/g')
